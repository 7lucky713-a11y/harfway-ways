...
const DEFAULT_ANALYTICS_SERVICES=[
  ...
  {serviceName:'sale-watch',label:'SALE WATCH',contentType:'public_tool',productionUrl:'https://harfway-playback.vercel.app/sales',hosts:['harfway-playback.vercel.app','harfway-playback-harf-way.vercel.app'],source:'built_in',expectedEvents:['page_view','store_click','content_click','sale_filter','sale_search']},
...
async function fetchJson(url,timeoutMs=2500){
  ...
  try{
    const response=await fetch(url,{cache:'no-store',signal:ctrl.signal,headers:{'user-agent':'HARF-WAY-Analytics-Registry/1.1'}});
...
function normalizeServiceRoute(service){
  const productionUrl=String(service?.productionUrl||'').trim();
  const explicitPath=String(service?.pathPrefix||'').trim();
  return {...service,pathPrefix:cleanPath(explicitPath||pathFromUrl(productionUrl))};
}
...
function fromManifest(manifest,item={}){
  ...
  return normalizeServiceRoute({
    ...
    productionUrl,
    pathPrefix:String(analytics.path_prefix||'').trim(),
    hosts:[...new Set(hosts)],
    source:'manifest',
...
export async function getAnalyticsRegistry(){
  const services=DEFAULT_ANALYTICS_SERVICES.map(x=>normalizeServiceRoute({...x,hosts:[...x.hosts]}));
  const usedNames=new Set(services.map(x=>x.serviceName));
  const usedRoutes=new Set(services.flatMap(x=>x.hosts.map(host=>`${host}|${x.pathPrefix}`)));
  ...
  for(const {item,manifest} of discovered){
    ...
    service.hosts=service.hosts.filter(host=>!usedRoutes.has(`${host}|${service.pathPrefix}`));
    if(!service.hosts.length)continue;
    services.push(service);
    usedNames.add(service.serviceName);
    service.hosts.forEach(host=>usedRoutes.add(`${host}|${service.pathPrefix}`));
    autoAdded++;
  }
  return {
    services,
