...
function listSidebar(notes){
 const counts=new Map();for(const n of notes){const g=n.gameName||'ゲーム名なし';counts.set(g,(counts.get(g)||0)+1)}
 const items=[...counts].sort((a,b)=>b[1]-a[1]).slice(0,20);
 return `<aside class="sidebar"><section class="side-block"><p class="side-label">ABOUT</p><h2>遊んでいる途中の記録。</h2><p>完成した記事ほどまとまっていなくても、あとから読み返したくなる断片があります。ここは、そんな言葉を置いておく場所です。</p></section><section class="side-block"><p class="side-label">ゲームから探す</p><div class="filter"><button type="button" class="chosen" data-game-filter="all" aria-pressed="true">すべてのゲーム <span>${notes.length}</span></button>${items.map(([g,count])=>`<button type="button" data-game-filter="${escapeHtml(g)}" aria-pressed="false">${escapeHtml(g)} <span>${count}</span></button>`).join('')}</div></section><section class="side-block"><p class="side-label">もっと読む</p><a class="side-link" href="/words/">用語解説へ <span>↗</span></a></section></aside>`;
}

function listBody(notes,demo=false){
 return `<main><section class="intro wrap"><p class="eyebrow">遊んでいる途中に、書きとめたこと。</p><h1>プレイノート</h1><p class="intro-copy">遊んで、考えて、引っかかったことを少しずつ。レビューになる前の感想や、まだ答えの出ていないメモを並べています。</p></section><div class="content-layout wrap"><section class="feed" aria-label="最近の記録"><div class="section-heading"><h2>最近の記録</h2><span>新しい順</span></div>${notes.length?notes.map((n,i)=>listItem(n,i,demo)).join(''):'<p class="empty">公開中のプレイノートはまだありません。</p>'}<p class="empty" id="filter-empty" hidden>このゲームの公開ノートはまだありません。</p></section>${listSidebar(notes)}</div></main>`;
}

function bodyHtml(value){
 const parts=String(value||'').replace(/\r\n?/g,'\n').trim().split(/\n\s*\n/).map(s=>s.trim()).filter(Boolean);
 const lead=parts.length>=2?parts.shift():null;
 const render=s=>escapeHtml(s).replace(/\n/g,'<br>');
 return {lead:lead?`<div class="lead"><p>${render(lead)}</p></div>`:'',body:`<div class="article-body">${parts.map(s=>`<p>${render(s)}</p>`).join('')||'<p>本文はまだありません。</p>'}</div>`};
}
...
async function handler(req,res){
 ...
 try{
  ...
  if(!requestedId){
   ...
   return res.status(200).end(pageShell(seoHead(req,{title,description,path:'/notes/',type:'website',schema}),listBody(notes,demo),{demo,filters:true}));
  }
