...
function steamResultFromTag(tag,query){
  ...
  const title=strip(tag.match(/<span[^>]*class=["'][^"']*title[^"']*["'][^>]*>([\s\S]*?)<\/span>/i)?.[1]||tag.match(/<div[^>]*class=["'][^"']*match_name[^"']*["'][^>]*>([\s\S]*?)<\/div>/i)?.[1]||'');
...
async function steamSuggest(query){
  ...
  for(const url of urls){
    try{
      ...
      if(!r.ok)continue;const html=await r.text(),out=[],seen=new Set();
      const tags=[...html.matchAll(/<a\b[^>]*(?:match|search_result_row)[^>]*>[\s\S]*?<\/a>/gi)].map(m=>m[0]);
      for(const tag of tags){const item=steamResultFromTag(tag,query);if(!item||seen.has(item.appId))continue;seen.add(item.appId);out.push(item)}
      if(!out.length){
        for(const m of html.matchAll(/href=["']https?:\/\/store\.steampowered\.com\/app\/(\d+)\/[^"']*["'][^>]*>[\s\S]{0,800}?(?:match_name|title)[^>]*>([\s\S]*?)<\//gi)){
          const appId=m[1],title=strip(m[2]);if(!title||seen.has(appId))continue;seen.add(appId);out.push({provider:'steam',appId,title,url:`https://store.steampowered.com/app/${appId}/`,score:similarity(query,title)})
        }
      }
      if(out.length)return out.sort((a,b)=>b.score-a.score).slice(0,8);
    }catch{}
  }
  return [];
}
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(200).json({ok:true,provider,query:q,count:candidates.length,candidates});
