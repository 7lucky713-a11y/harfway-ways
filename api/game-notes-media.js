...
const PREFIX = 'private-game-notes';
...
function validFinalKey(value) {
  ...
  return /^private-game-notes\/\d{4}-\d{2}-\d{2}\/[0-9a-f-]+\.(jpg|png|webp|gif|mp4|webm)$/i.test(key) ? key : '';
}
function envStatus() {
  return {
...
async function startUpload(req, res) {
  ...
  const client = r2(); const bkt = bucket();
  await cleanupStale(client, bkt).catch(() => undefined);
  const uploadId = crypto.randomUUID();
  await client.send(new PutObjectCommand({ Bucket: bkt, Key: reservationKey(uploadId), Body: JSON.stringify({ fileName, contentType, size, createdAt: new Date().toISOString() }), ContentType: 'application/json', CacheControl: 'no-store' }));
...
async function completeUpload(req, res) {
  ...
  const reservation = await readReservation(client, bkt, uploadId);
  if (!validReservation(reservation, contentType, size)) return json(res, 409, { ok: false, error: 'upload_session_invalid' });
  const buffers = [], tempObjects = []; let total = 0;
  for (let part = 1; part <= parts; part += 1) {
    ...
    buffers.push(buffer); tempObjects.push({ Key: key });
  }
  ...
  return json(res, 200, { ok: true, media: { key, type: contentType, kind: info.kind, size, name: fileName } });
}
...
export default async function handler(req, res) {
  try {
    ...
    console.error('[game-notes-media]', error?.message || error);
    const denied = /access\s*denied/i.test(String(error?.message || '')) || String(error?.name || '') === 'AccessDenied';
    return json(res, denied ? 403 : (error?.status || 500), { ok: false, error: denied ? 'r2_write_permission_denied' : (error?.message || 'game_notes_media_failed') });
  }
}
