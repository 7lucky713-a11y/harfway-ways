...
function getPreviewDatabaseUrl() {
  return process.env.SALVAGER_PREVIEW_DATABASE_URL || '';
}
...
function cleanString(value, max = 150000) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const url = cleanString(article.url, 4000);
    const title = cleanString(article.title, 1000);
    if (!url || !title) return res.status(400).json({ ok: false, error: 'article_required' });

    const parsed = new URL(url);
    if (!/(^|\.)harf-way\.com$/i.test(parsed.hostname)) {
      return res.status(400).json({ ok: false, error: 'harf_way_url_only' });
    }

    const contentId = makeId('content', url);
    const links = Array.isArray(payload.links) ? payload.links.slice(0, 100) : [];
    const assets = Array.isArray(article.images) ? article.images.slice(0, 100) : [];
    ...
    const rows = await sql`
      SELECT id, title, url, status, games, assets, updated_at
