...
function paragraphize(text=''){
  ...
  for(const sentence of sentences){
    ...
    if(buf && (buf.length+sentence.length>300 || /[。！？!?]$/.test(buf)&&buf.length>180)){parts.push(buf.trim());buf='';}
    buf+=sentence;
  }
...
function extractReadableBlocks(html=''){
  ...
  for(const m of String(html).matchAll(/<(p|h[2-6]|li|blockquote)\b[^>]*>([\s\S]*?)<\/\1>/gi)){
    ...
    const key=text.normalize('NFKC');if(seen.has(key))continue;
    seen.add(key);out.push(text);
  }
  return paragraphize(out.join('\n\n')).slice(0,150000);
}

function pick(html,patterns){for(const re of patterns){const m=String(html).match(re);if(m?.[1])return stripHtml(m[1]);}return ''}
function attr(tag,name){const m=String(tag).match(new RegExp(`${name}=["']([^"']*)["']`,'i'));return m?.[1]?decodeEntities(m[1]):''}
...
function hintKey(value=''){return cleanCandidate(value).normalize('NFKC').toLowerCase().replace(/[\s:：・!！?？'"“”‘’()（）\[\]【】_\-–—～~]+/g,'')}
function sameStore(a='',b=''){
  if(!a||!b)return false;
  try{const ua=new URL(a),ub=new URL(b);if(ua.hostname!==ub.hostname)return false;const sa=ua.pathname.match(/\/app\/(\d+)/)?.[1]||'',sb=ub.pathname.match(/\/app\/(\d+)/)?.[1]||'';if(sa&&sb)return sa===sb;return ua.origin+ua.pathname.replace(/\/$/,'')===ub.origin+ub.pathname.replace(/\/$/,'')}catch{return a===b}
}
function looksSameGame(a,b){const ka=hintKey(a?.name),kb=hintKey(b?.name);if(!ka||!kb)return false;if(ka===kb||sameStore(a?.url,b?.url))return true;const shorter=ka.length<=kb.length?ka:kb,longer=ka.length>kb.length?ka:kb;return shorter.length>=5&&longer.includes(shorter)}
function mergeHint(prev,item){const preferred=(item.confidence>prev.confidence)||(item.confidence===prev.confidence&&item.name.length>prev.name.length)?item:prev;const other=preferred===item?prev:item;return {...other,...preferred,url:preferred.url||other.url||'',confidence:Math.max(Number(prev.confidence||0),Number(item.confidence||0)),sources:[...new Set([...(prev.sources||[prev.source]).filter(Boolean),...(item.sources||[item.source]).filter(Boolean)])]}}
function uniqueHints(items){const result=[];for(const raw of items){const name=cleanCandidate(raw.name||'');if(!name||name.length<2||name.length>120)continue;const item={...raw,name,confidence:Number(raw.confidence||hintConfidence(raw.source)),sources:[raw.source].filter(Boolean)};const i=result.findIndex(prev=>looksSameGame(prev,item));if(i>=0)result[i]=mergeHint(result[i],item);else result.push(item)}return result.sort((a,b)=>b.confidence-a.confidence).slice(0,120)}

function addTitleHints(items,title){
  const t=cleanCandidate(title);if(!t)return;
  for(const m of t.matchAll(/[『「【《]([^』」】》]{2,120})[』」】》]/g)){const full=cleanCandidate(m[1]);if(!full)continue;items.push({name:full,source:'page-title',url:'',confidence:92});const short=full.split(/\s+[–—-]\s+/)[0]?.trim();if(short&&short!==full)items.push({name:short,source:'page-title-short',url:'',confidence:100})}
  for(const m of t.matchAll(/["“‘']([^"”’']{2,100})["”’']/g))items.push({name:m[1],source:'page-title',url:'',confidence:90});
}
function addPageSlugHint(items,baseUrl){try{const u=new URL(baseUrl),seg=u.pathname.split('/').filter(Boolean),slug=seg.at(-1)||'';if(slug&&slug.length>=2&&!/^(game|games|weekly|yorimichi|article|articles)$/i.test(slug))items.push({name:decodeURIComponent(slug).replace(/[-_]+/g,' '),source:'page-slug',url:'',confidence:82})}catch{}}
...
function extractScriptGameHints(html,items){const raw=String(html).replace(/\\\//g,'/').replace(/&quot;/gi,'"').replace(/&#39;/gi,"'");for(const m of raw.matchAll(/https?:\/\/store\.steampowered\.com\/app\/\d+\/[^\s"'<>\\)]+/gi))addStoreHint(items,m[0],'');for(const m of raw.matchAll(/(?:title|name)\s*:\s*(["'`])([^"'`]{2,120})\1[\s\S]{0,500}?(?:steam|storeUrl|store_url)\s*:\s*(["'`])(https?:\/\/[^"'`]+)\3/gi))addStoreHint(items,m[4],m[2])}
function extractNameHints(html,baseUrl,pageTitle){const items=[];addTitleHints(items,pageTitle);addPageSlugHint(items,baseUrl);const scope=String(html).match(/<article[^>]*>([\s\S]*?)<\/article>/i)?.[1]||html;for(const m of scope.matchAll(/<h([1-6])[^>]*>([\s\S]*?)<\/h\1>/gi)){const name=cleanCandidate(m[2]);if(name)items.push({name,source:`heading-h${m[1]}`,url:'',confidence:78})}for(const m of scope.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi))addStoreHint(items,absoluteUrl(decodeEntities(m[1]),baseUrl),m[2]);extractScriptGameHints(html,items);return uniqueHints(items)}

function ignoredExternalHost(host=''){return /(^|\.)(twitter\.com|x\.com|youtube\.com|youtu\.be|facebook\.com|instagram\.com|tiktok\.com|amazon\.|amzn\.to|note\.com|discord\.|discordapp\.com)$/i.test(host)}
function extractPlatformLinks(html,baseUrl){
  const scope=String(html).match(/<article[^>]*>([\s\S]*?)<\/article>/i)?.[1]||html;
  const out=[],seen=new Set();let baseHost='';try{baseHost=new URL(baseUrl).hostname.toLowerCase()}catch{}
  for(const m of scope.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){
    ...
    const anchor=cleanCandidate(m[2]);let classified=classifyGameLink(raw);if(!classified)continue;
...
function fieldPenalty(path=''){const p=path.toLowerCase();let n=0;if(/(?:^|\.)(?:link|guid|slug|date|modified|status|type|template|author|id)$/.test(p))n+=1000;if(/_links|yoast|rank_math|jetpack|media|image|thumbnail|avatar|caption|alt_text/.test(p))n+=700;if(/excerpt/.test(p))n+=180;return n}
function fieldBonus(path=''){const p=path.toLowerCase();let n=0;if(/acf|custom|meta/.test(p))n+=180;if(/body|content|text|description|summary|review|comment|note|intro|point|catch|scene|memo/.test(p))n+=150;return n}
function textScore(text='',path=''){const t=String(text).trim();if(!t||isBoilerplate(t))return -9999;const jp=(t.match(/[ぁ-んァ-ヶ一-龯]/g)||[]).length,urls=(t.match(/https?:\/\//g)||[]).length;return Math.min(t.length,7000)+Math.min(jp,1800)*1.6+fieldBonus(path)-fieldPenalty(path)-urls*180}
function collectTextFields(value,path='',out=[],depth=0){if(depth>7||value==null)return out;if(typeof value==='string'){const text=cleanText(value);if(text&&text.length>=40)out.push({path,text,score:textScore(text,path),order:out.length});return out}if(Array.isArray(value)){value.forEach((v,i)=>collectTextFields(v,`${path}[${i}]`,out,depth+1));return out}if(typeof value==='object')for(const [k,v] of Object.entries(value))collectTextFields(v,path?`${path}.${k}`:k,out,depth+1);return out}
function selectWordPressText(row){
  ...
  const pool=preferred.length?preferred:fields;if(!pool.length)return {text:'',source:'none',fields:[]};
  const best=pool[0],selected=[best];
  for(const candidate of pool.slice(1,8)){if(selected.length>=4)break;if(candidate.score<best.score*.45)continue;const c=candidate.text.normalize('NFKC');if(selected.some(x=>x.text.normalize('NFKC').includes(c)||c.includes(x.text.normalize('NFKC'))))continue;if(candidate.path.split('.')[0]!==best.path.split('.')[0])continue;selected.push(candidate)}
  selected.sort((a,b)=>a.order-b.order);const body=paragraphize(selected.map(x=>x.text).join('\n\n')).slice(0,150000);
...
function extractJsonLdText(html=''){const candidates=[];for(const m of String(html).matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)){try{const data=JSON.parse(m[1]),stack=Array.isArray(data)?data:[data];while(stack.length){const x=stack.pop();if(!x||typeof x!=='object')continue;if(typeof x.articleBody==='string')candidates.push({path:'jsonld.articleBody',text:cleanText(x.articleBody)});if(Array.isArray(x['@graph']))stack.push(...x['@graph'])}}catch{}}return candidates.sort((a,b)=>b.text.length-a.text.length)[0]||null}
async function fetchJson(url){try{const r=await fetch(url,{headers:{'user-agent':'HARF-WAY Archive Salvager/1.0','accept':'application/json'},redirect:'follow'});if(!r.ok)return null;return await r.json()}catch{return null}}
async function fetchWordPressRecord(pageUrl){
  const u=new URL(pageUrl),parts=u.pathname.split('/').filter(Boolean),slug=parts.at(-1)||'',route=parts[0]||'';if(!slug)return null;
  const candidates=[];const typeData=await fetchJson(`${u.origin}/wp-json/wp/v2/types`);
  if(typeData&&typeof typeData==='object'){const types=Object.values(typeData).sort((a,b)=>((a?.rest_base===route||a?.slug===route)?0:1)-((b?.rest_base===route||b?.slug===route)?0:1));for(const t of types){const base=String(t?.rest_base||'').trim();if(base)candidates.push(base)}}
  ...
  for(const restBase of [...new Set(candidates)]){
    ...
    const rows=await fetchJson(`${u.origin}/wp-json/wp/v2/${restBase}?slug=${encodeURIComponent(slug)}&per_page=1`);if(!Array.isArray(rows)||!rows[0])continue;
    ...
    if(row.featured_media){const media=await fetchJson(`${u.origin}/wp-json/wp/v2/media/${row.featured_media}?_fields=source_url,alt_text`),src=String(media?.source_url||'');if(src&&!images.some(x=>x.url===src))images.unshift({url:src,alt:String(media?.alt_text||''),featured:true})}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const response=await fetch(url.toString(),{headers:{'user-agent':'HARF-WAY Archive Salvager/1.0'},redirect:'follow'});if(!response.ok)return res.status(502).json({ok:false,error:`source_${response.status}`});
