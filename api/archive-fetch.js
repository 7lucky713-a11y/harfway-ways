...
function addTitleHints(items,title){
  ...
  const quoted=[...t.matchAll(/["“‘']([^"”’']{2,100})["”’']/g)];
  for(const m of quoted)items.push({name:m[1],source:'page-title',url:'',confidence:90});
}
...
function addStoreHint(items,href,text=''){
  ...
  if(cleanText&&!/^(store page|steam|store|official|公式|購入|販売ページ)$/i.test(cleanText))items.push({name:cleanText,source:'store-link-text',url:href,confidence:98});
  if(host==='store.steampowered.com'){
    const seg=u.pathname.split('/').filter(Boolean);const appIndex=seg.findIndex(x=>x==='app');
...
function extractNameHints(html,baseUrl,pageTitle){
  ...
  const articleMatch=String(html).match(/<article[^>]*>([\s\S]*?)<\/article>/i);const scope=articleMatch?.[1]||html;
