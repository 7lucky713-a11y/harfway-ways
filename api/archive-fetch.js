...
export default async function handler(req,res){
  ...
  try{
    ...
    const response=await fetch(url.toString(),{headers:{'user-agent':'HARF-WAY Archive Salvager/0.1'},redirect:'follow'});
    ...
    return res.status(200).json({ok:true,article:{url:response.url||url.toString(),title,date,text:body.slice(0,120000)}});
  }catch(error){
