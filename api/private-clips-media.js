...
function safeName(value) {
  return String(value || 'image').replace(/[\u0000-\u001f]/g, '').slice(0, 180) || 'image';
}
...
function validFinalKey(value) {
  ...
  return /^private-clips\/\d{4}-\d{2}-\d{2}\/[0-9a-f-]+\.(jpg|png|webp|gif)$/i.test(key) ? key : '';
}
...
async function authorize(req) {
  ...
  if (auth.ok) return;
  const e = new Error(auth.error || 'unauthorized');
  e.status = auth.status || 401;
  throw e;
}
function r2() {
...
function tempKey(uploadId, part) { return `${PREFIX}/tmp/${uploadId}/${part}.part`; }
function tempPrefix(uploadId) { return `${PREFIX}/tmp/${uploadId}/`; }
function reservationKey(uploadId) { return `${PREFIX}/guard/open/${uploadId}.json`; }
function finalKey(uploadId, ext) { return `${PREFIX}/${new Date().toISOString().slice(0, 10)}/${uploadId}.${ext}`; }
...
async function cleanupStale(client, bkt) {
  const listed = await client.send(new ListObjectsV2Command({ Bucket: bkt, Prefix: `${PREFIX}/guard/open/`, MaxKeys: 100 }));
  const keys = [];
...
async function startUpload(req, res) {
  ...
  if (!Number.isInteger(size) || size <= 0 || size > info.max) return json(res, 413, { ok: false, error: 'image_too_large', maxBytes: info.max });
  const client = r2();
  const bkt = bucket();
  ...
  return json(res, 200, { ok: true, uploadId, chunkBytes: CHUNK_BYTES, maxBytes: info.max, kind: 'image' });
}
async function putPart(req, res) {
  ...
  if (!Number.isInteger(size) || size <= 0 || size > info.max) return json(res, 413, { ok: false, error: 'image_too_large' });
  const client = r2();
  const bkt = bucket();
  ...
  await client.send(new PutObjectCommand({ Bucket: bkt, Key: tempKey(uploadId, part), Body: bytes, ContentType: 'application/octet-stream', CacheControl: 'no-store' }));
...
async function completeUpload(req, res) {
  ...
  for (let part = 1; part <= parts; part += 1) {
    ...
    total += buffer.length;
    if (total > info.max) throw new Error('image_too_large');
    buffers.push(buffer);
    tempObjects.push({ Key: key });
  }
  if (total !== size) throw new Error('uploaded_size_mismatch');
  const key = finalKey(uploadId, info.ext);
  await client.send(new PutObjectCommand({ Bucket: bkt, Key: key, Body: Buffer.concat(buffers), ContentType: contentType, CacheControl: 'private, no-store' }));
  ...
  return json(res, 200, { ok: true, media: { key, type: contentType, kind: 'image', size, name: fileName } });
}
async function deleteMedia(req, res) {
  const key = validFinalKey(parseBody(req).key);
  if (!key) return json(res, 400, { ok: false, error: 'invalid_media_key' });
  await r2().send(new DeleteObjectCommand({ Bucket: bucket(), Key: key }));
  return json(res, 200, { ok: true, deleted: true });
}
async function getMedia(req, res) {
  const key = validFinalKey(req.query?.key);
  if (!key) return json(res, 400, { ok: false, error: 'invalid_media_key' });
  const obj = await r2().send(new GetObjectCommand({ Bucket: bucket(), Key: key }));
  if (!obj.Body) return json(res, 404, { ok: false, error: 'media_not_found' });
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'file') return await getMedia(req, res);
      const flags = {
      ...
      return json(res, configured ? 200 : 503, { ok: configured, configured, environment: process.env.VERCEL_ENV || 'development', env: flags, maxImageBytes: MAX_BYTES, chunkBytes: CHUNK_BYTES });
    }
    ...
    console.error('[private-clips-media]', error?.message || error);
    const denied = /access\s*denied/i.test(String(error?.message || '')) || String(error?.name || '') === 'AccessDenied';
    return json(res, denied ? 403 : (error?.status || 500), { ok: false, error: denied ? 'r2_write_permission_denied' : (error?.message || 'private_clips_media_failed') });
  }
}
