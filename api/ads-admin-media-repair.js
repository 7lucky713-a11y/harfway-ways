...
async function sessionAuthorization(req) {
  ...
  const response = await fetch(`${AUTH_UPSTREAM}/get-session`, {
    ...
    headers: {
      ...
      'user-agent': req.headers['user-agent'] || 'HARF-WAY-ADS-Media-Repair/1.0'
    },
    redirect: 'manual'
  });
...
async function mediaApi(action, authorization, body) {
  ...
  if (!response.ok) throw new Error(data?.error || data?.message || `R2 ${action} failed (${response.status})`);
  return data;
}

async function uploadToR2(media, campaignId, authorization) {
  const metadata = {
    campaignId,
  ...
  for (let index = 0; index < parts; index += 1) {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.error || data?.message || `R2 part failed (${response.status})`);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const completed = await uploadToR2(media, campaignId, authorization);
    ...
    console.error('[ads-admin-media-repair] failed', String(error?.message || error));
    return res.status(500).json({ ok: false, error: String(error?.message || 'repair_failed') });
  }
}
