...
function patchCleanLayout(){
  ...
  const newLimits="vertical=window.__MINIBOOK_DESIGN__?.writingMode==='vertical',density=window.__MINIBOOK_DESIGN__||{},scale=15/bodySize,layoutScale=Math.max(.34,Math.min(1,Number(window.__MINIBOOK_LAYOUT__?.capacityScale||1))),gridUnits=vertical?Math.max(14,Math.min(48,Number(density.verticalCharsPerColumn||28))):Math.max(14,Math.min(40,Number(density.horizontalCharsPerLine||26))),gridCount=vertical?Math.max(5,Math.min(18,Number(density.verticalColumns||12))):Math.max(10,Math.min(26,Number(density.horizontalLines||18))),gridCapacity=gridUnits*gridCount,firstRatio=vertical?.74:.62,firstLimit=Math.max(vertical?40:50,Math.min(vertical?520:560,Math.round(gridCapacity*firstRatio*scale*layoutScale))),nextLimit=Math.max(vertical?60:70,Math.min(vertical?760:860,Math.round(gridCapacity*scale*layoutScale)))";
