(() => {
  const ADS_MEDIA_API = 'https://design-stock.vercel.app/api/ads-media';
  ...
  function progress(message) {
    if (!progressEl) {
      ...
      progressEl.setAttribute('data-hw-ads-upload', '1');
  ...
  async function uploadToR2(row, authorization) {
    ...
    const completed = await api('complete', authorization, {
      ...metadata,
  ...
  window.fetch = async function harfwayAdsFetch(input, init) {
    ...
    try {
      ...
      if (!row || !row.campaign_id || !row.data_base64) return nativeFetch(input, init);

      const headers = mergedHeaders(input, init);
      const authorization = headers.get('authorization') || '';
      if (!/^Bearer\s+\S+/i.test(authorization)) throw new Error('ログイン情報を確認できませんでした。');

      await uploadToR2(row, authorization);
      ...
      return new Response('', {
        ...
        headers: {
          'Content-Type': 'application/json',
          'X-HARFWAY-MEDIA-STORAGE': 'r2'
        }
      });
    } catch (error) {
      clearProgress();
      console.error('[HARF-WAY ADS] R2 upload failed', error);
      return new Response(JSON.stringify({
        message: error instanceof Error ? error.message : '素材を保存できませんでした。'
      }), {
        status: 500,
