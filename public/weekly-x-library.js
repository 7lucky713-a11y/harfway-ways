(()=>{
  ...
  const addPost=(url,title='',note='')=>{
    ...
    xPosts.unshift({id:`x-${statusId(url)||Date.now()}`,url,title:String(title||'').trim()||`X POST ${statusId(url)?'#'+statusId(url).slice(-6):''}`,note:String(note||'').trim(),addedAt:new Date().toISOString()});persist();return true;
  };
  const syncAssigned=()=>{
    let changed=false;
    try{gameIds.forEach(id=>{const url=String(gameEdit?.[id]?.xUrl||'').trim();if(validX(url)&&!xPosts.some(p=>p.url===url)){const g=item(id);xPosts.push({id:`x-${statusId(url)||Date.now()}`,url,title:g?.title||'X POST',note:'GAME LOGから取得',addedAt:new Date().toISOString()});changed=true}})}catch{}
    if(changed)persist();
  };
  ...
  renderCards=function(){
    ...
    cards.innerHTML=list.map(p=>{
      const options=gameIds.length?gameIds.map(id=>`<option value="${esc(id)}">${esc(item(id)?.title||id)}</option>`).join(''):'<option value="">先にGAME LOGへゲームを追加</option>';
      return `<article class="card x-source-card"><div class="thumb x-thumb">X / TWITTER SOURCE</div><div class="cardbody"><div class="badges"><span class="badge">X POST</span><span class="badge">SOURCE LIBRARY</span></div><div class="title">${esc(p.title)}</div>${p.note?`<div class="summary">${esc(p.note)}</div>`:''}<div class="x-source-url">${esc(p.url)}</div><div class="x-assign"><select class="select" data-xgame="${esc(p.id)}">${options}</select><button class="mini primary" data-xassign="${esc(p.id)}">ゲームへ割当</button></div><div class="cardactions"><a class="mini" href="${esc(p.url)}" target="_blank" rel="noopener">OPEN ↗</a><button class="mini" data-xremove="${esc(p.id)}">削除</button></div></div></article>`;
    }).join('');
  };
  ...
  document.getElementById('cards').onclick=e=>{
    ...
    if(assign){
      ...
      if(!p||!gid){flash('先にWEEKDAY GAME LOGへゲームを追加してください');return}
      gameEdit[gid]={...(gameEdit[gid]||{}),xUrl:p.url};gameTab[gid]='x';renderGamesSelected();decorateGameTabs();renderPreview();flash(`${item(gid)?.title||'ゲーム'} にX投稿を割り当てました`);return;
    }
