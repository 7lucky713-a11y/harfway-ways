(() => {
  ...
  function installStyles(){
    ...
    const style=document.createElement('style');style.id='glossary-publication-styles';style.textContent=`
      ...
      @media(max-width:700px){.glossary-publication button,.glossary-publication a{flex:1;text-align:center;min-width:130px}}
  ...
  function render(){
    ...
    else{root.innerHTML=`<span class="glossary-publication-state">非公開</span><button type="button" class="primary" data-glossary-public-action="publish">読者向けに公開</button><span class="glossary-publication-help">現在の保存済み内容から「ことばノート」用snapshotを作ります。Private Glossary自体は公開されません。</span>`}
  }
  ...
  async function onAction(event){
    ...
    const message=action==='publish'?'現在の保存済み内容を「ことばノート」として公開します。編集中の変更がある場合は、先に「保存する」で確定してください。':'現在の保存済み内容で公開snapshotを更新します。編集中の変更は含まれません。';
  ...
  installStyles();ensureControls();refresh();
