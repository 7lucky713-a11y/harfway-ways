(() => {
  ...
  function demoAd() {
    return {
      id: 'sale-watch-preview-demo',
      title: 'セールを見ている人へ届けるPR枠',
      catchText: 'SALE WATCHの流れに自然に差し込む、購入直前のプロモーション枠です。',
      description: 'Preview sample / impression is not counted.',
      storeUrl: '#',
      targetTags: ['PROMOTED', 'SALE WATCH'],
      mediaUrl: '',
      mediaMime: '',
      __demo: true,
    };
  ...
  function addStyles() {
    if (document.getElementById('sale-ads-style')) return;
    ...
    s.textContent = `
      ...
      .sale-ad-body{padding:13px;display:flex;flex-direction:column;gap:12px;min-height:238px}
      .sale-ad-kicker{color:#eaff35;font:950 9px/1.2 system-ui,sans-serif;letter-spacing:.14em}
      .sale-ad-title{margin:0;font:950 22px/1.08 system-ui,sans-serif;letter-spacing:-.035em;color:#fff}
      .sale-ad-copy{margin:0;color:#bdc5ae;font:700 11px/1.7 system-ui,sans-serif}
      .sale-ad-tags{display:flex;gap:5px;flex-wrap:wrap}
      .sale-ad-tag{border:1px solid #4e5b2a;border-radius:999px;padding:4px 6px;color:#cfdc7c;font:800 8px/1.2 system-ui,sans-serif}
      ...
      .sale-ad-note{color:#7f8972;font:800 8px/1.35 system-ui,sans-serif;text-align:right}
      .sale-ad-note.preview{color:#ffd06d}
  ...
  function mediaHtml(a) {
    if (!a.mediaUrl) return '<div class="sale-ad-fallback">PROMOTED<br>GAME</div>';
    if ((a.mediaMime || '').startsWith('video/')) {
      return `<video muted autoplay loop playsinline preload="metadata" src="${esc(a.mediaUrl)}"></video>`;
    }
    return `<img src="${esc(a.mediaUrl)}" alt="" loading="lazy">`;
  }

  function cardHtml() {
    const tags = Array.isArray(ad?.targetTags) ? ad.targetTags.slice(0, 4) : [];
    const store = ad?.storeUrl && ad.storeUrl !== '#'
      ? `<a class="sale-ad-store" data-sale-ad-store href="${esc(ad.storeUrl)}" target="_blank" rel="noopener">ストアで見る ↗</a>`
      : '<span></span>';
    const note = (!TRACK_ENABLED || ad?.__demo)
      ? '<span class="sale-ad-note preview">PREVIEW / NOT COUNTED</span>'
      : '<span class="sale-ad-note">SPONSORED</span>';
    return `<article class="card sale-ad-card" data-sale-ad="1">
      <div class="sale-ad-media">${mediaHtml(ad)}<span class="sale-ad-badge">PR / SPONSORED</span></div>
      <div class="sale-ad-body">
        <div class="sale-ad-kicker">HARF-WAY / SALE WATCH PR</div>
        <h2 class="sale-ad-title">${esc(ad?.title || 'PROMOTED')}</h2>
        <p class="sale-ad-copy">${esc(ad?.catchText || ad?.description || '')}</p>
        ${tags.length ? `<div class="sale-ad-tags">${tags.map((t) => `<span class="sale-ad-tag"># ${esc(t)}</span>`).join('')}</div>` : ''}
        <div class="sale-ad-actions">${store}${note}</div>
      </div>
    </article>`;
  }

  function observeImpression(card) {
    observer?.disconnect();
        ...
        if (entry.intersectionRatio >= 0.5) {
          if (!timer) {
            timer = setTimeout(async () => {
              timer = null;
              if (counted || !card.isConnected) return;
              if (!TRACK_ENABLED || ad?.__demo) {
                counted = true;
                return;
              }
              const result = await record('impression');
  ...
  function inject() {
    if (injecting || !ad) return;
    const grid = document.querySelector('#grid');
    if (!grid || grid.querySelector('[data-sale-ad]')) return;
    const cards = [...grid.querySelectorAll(':scope > .card:not(.sale-ad-card)')];
    if (cards.length < everyN) return;
    injecting = true;
    try {
      cards[everyN - 1].insertAdjacentHTML('afterend', cardHtml());
      const card = grid.querySelector('[data-sale-ad]');
      card?.querySelector('[data-sale-ad-store]')?.addEventListener('click', () => {
        record('click');
        record('store_visit');
      }, true);
