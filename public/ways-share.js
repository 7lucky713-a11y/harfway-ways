(() => {
  ...
  const xGameUrl = (game) => `${publicGameUrl(game)}?card=player-v1`;
  ...
  const iframeCode = (game) => `<div style="width:100%;max-width:560px;aspect-ratio:16/9;overflow:hidden"><iframe src="${embedUrl(game)}" loading="lazy" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen scrolling="no" style="display:block;width:100%;height:100%;border:0"></iframe></div>`;
  ...
  function toast(message) {
    ...
    if (!node) {
      ...
      node.id = 'hwWaysShareToast';
      node.className = 'hw-share-toast';
...
`;
  ...
  function open(game) {
    ...
    note.textContent = IS_PRODUCTION
      ? 'Xには動画Player Cardを要求します。X側の仕様・キャッシュ状況によっては画像にフォールバックする場合があります。'
      : 'PreviewはVercel認証で保護されているため、Xの動画カード確認だけは本番公開後に行います。埋め込み表示はこのPreviewで確認できます。';
    modal.classList.add('on');
    document.body.style.overflow = 'hidden';
    track('share_open', game);
  }
  ...
  async function action(kind) {
    ...
    const text = `WAYSで「${game.title}」を見る`;
    try {
      if (kind === 'copy') {
        await copyText(url);
        toast(IS_PRODUCTION ? '共有リンクをコピーしました' : 'Previewリンクをコピーしました');
      } else if (kind === 'embed') {
        await copyText(iframeCode(game));
        toast(IS_PRODUCTION ? 'レスポンシブ埋め込みをコピーしました' : 'Preview用の埋め込みをコピーしました');
      } else if (kind === 'x' && IS_PRODUCTION) {
        window.open(`https://x.com/intent/post?text=${encodeURIComponent(text)}&url=${encodeURIComponent(xGameUrl(game))}`, '_blank', 'noopener,noreferrer');
      } else if (kind === 'x') {
        toast('Xの動画カードは本番公開後に確認できます');
        return;
      }
  ...
  function button(game) {
    ...
    b.type = 'button';
    b.className = SHARE_CLASS;
    b.textContent = innerWidth < 900 ? 'SHARE' : '共有';
    b.dataset.gameId = game.id;
    b.setAttribute('aria-label', `${game.title || 'この作品'}を共有`);
    b.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      open(byId.get(b.dataset.gameId) || game);
    });
    return b;
  }
  ...
  function decorateDesktop() {
    ...
    if (!box) return;
    const game = desktopGame();
    const old = box.querySelector(`.${SHARE_CLASS}`);
    if (!game?.id) { old?.remove(); return; }
    if (old?.dataset.gameId === game.id) return;
    old?.remove();
    const b = button(game);
    const take = box.querySelector('.hw-take-home');
    const article = box.querySelector('.article');
    const store = box.querySelector('.store');
    if (take) take.insertAdjacentElement('afterend', b);
    else if (article) article.insertAdjacentElement('afterend', b);
    else if (store) store.insertAdjacentElement('afterend', b);
    else box.appendChild(b);
  }

  function decorateMobile() {
    document.querySelectorAll('#mfeed .m-card:not(.ways-ad-mobile)').forEach(card => {
      const box = card.querySelector('.m-meta');
      if (!box) return;
      const game = mobileGame(card);
      const old = box.querySelector(`.${SHARE_CLASS}`);
      if (!game?.id) { old?.remove(); return; }
      if (old?.dataset.gameId === game.id) return;
      old?.remove();
      const b = button(game);
      const take = box.querySelector('.hw-take-home');
      const store = box.querySelector('a[href]');
      if (take) take.insertAdjacentElement('afterend', b);
      else if (store) store.insertAdjacentElement('afterend', b);
      else box.appendChild(b);
    });
