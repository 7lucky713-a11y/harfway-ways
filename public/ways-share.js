(() => {
  ...
  const gameUrl = (game) => `${location.origin}/share/${encodeURIComponent(game.id)}`;
  const embedUrl = (game) => `${location.origin}/embed/${encodeURIComponent(game.id)}`;
  ...
  function track(action, game) {
    ...
    const payload = JSON.stringify({
      ...
      source: location.hostname === 'harfway-playback.vercel.app' ? 'ways' : 'ways-preview',
  ...
  function toast(message) {
    ...
    node._timer = setTimeout(() => node.classList.remove('on'), 1500);
  }
...
`;
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `<section class="hw-share-panel" role="dialog" aria-modal="true" aria-label="WAYSを共有"><div class="hw-share-head"><div><div class="hw-share-kicker">SHARE / WAYS</div><div class="hw-share-title" data-share-title>WAYS</div></div><button class="hw-share-close" type="button" aria-label="閉じる">×</button></div><div class="hw-share-body"><div class="hw-share-actions"><button class="hw-share-action primary" type="button" data-share-action="copy">リンクをコピー</button><button class="hw-share-action" type="button" data-share-action="native">端末で共有</button><button class="hw-share-action" type="button" data-share-action="x">Xで共有</button><button class="hw-share-action" type="button" data-share-action="embed">埋め込みをコピー</button></div><div class="hw-share-code"><label>SHARE URL</label><input readonly data-share-url></div><div class="hw-share-note">埋め込み先が対応している場合は動画プレイヤー、未対応の場合はサムネイル付きリンクとして表示されます。</div></div></section>`;
  ...
  async function action(kind) {
    ...
    try {
      if (kind === 'copy') { await copyText(url); toast('共有リンクをコピーしました'); }
      else if (kind === 'embed') { await copyText(iframeCode(game)); toast('埋め込みコードをコピーしました'); }
      else if (kind === 'native' && navigator.share) { await navigator.share({ title: `${game.title} | WAYS`, text, url }); }
      else if (kind === 'x') { window.open(`https://x.com/intent/post?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`, '_blank', 'noopener,noreferrer'); }
