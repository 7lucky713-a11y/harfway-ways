(() => {
  ...
  function markOriginalTabs(panel, heading, tabs) {
    ...
    if (host && host !== panel && !host.contains(heading)) {
      ...
      sources.forEach((button) => delete button.dataset.hwadsOriginalPreviewTab);
      return host;
    }
  ...
  function syncUi(panel, tabs) {
    const wrap = document.getElementById(UI_ID);
    if (!wrap) return;
    const active = activePlacement(tabs);
  ...
  function mount() {
    ...
    const tabHost = markOriginalTabs(panel, heading, tabs);

    let wrap = document.getElementById(UI_ID);
    if (!wrap) wrap = buildUi();

    const headerHost = tabHost && tabHost !== panel
      ? commonAncestor([heading, tabHost])
      : heading.parentElement;
    const anchor = headerHost && headerHost !== panel ? headerHost : heading.parentElement;
    if (wrap.parentElement !== panel && anchor?.parentElement) anchor.insertAdjacentElement('afterend', wrap);
    else if (!wrap.parentElement) panel.prepend(wrap);
  ...
  document.addEventListener('click', (event) => {
    ...
    if (deviceButton) {
      ...
      const panel = findPanel();
      if (!panel) return;
      setDevice(deviceButton.dataset.hwadsDevice, panel, originalTabs(panel));
      return;
    }
  ...
  style.id = 'hwads-inline-preview-layout-style';
  style.textContent = `
    ...
    #${UI_ID} button{appearance:none;min-height:42px;border:1px solid #d7cdbd;border-radius:10px;background:#fffdf8;color:#655946;padding:8px 10px;font:850 10px/1.25 Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;cursor:pointer;transition:background .16s ease,color .16s ease,border-color .16s ease,transform .16s ease}
    #${UI_ID} button:hover{border-color:#857661;transform:translateY(-1px)}
  ...
  const scheduleMount = () => {
    ...
    mountTimer = window.setTimeout(mount, 60);
  };
