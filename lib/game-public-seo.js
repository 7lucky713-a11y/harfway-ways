...
export async function listPublicNotes(sql){const rows=await sql`SELECT id,title,excerpt,body_text,metadata,created_at,updated_at FROM core.contents WHERE source=${NOTE_PUBLIC_SOURCE} AND content_type=${NOTE_PUBLIC_TYPE} AND status='active' ORDER BY COALESCE((metadata->>'publishedAt')::timestamptz,created_at) DESC`;const notes=rows.map(noteFromRow);const seen=new Map();for(const note of notes){const slug=notePublicSlug(note);if(seen.has(slug)){const e=new Error('duplicate_public_slug');e.status=409;throw e}seen.set(slug,note.id)}return notes}
