...
export function slugifyPublic(value,fallback='item',max=110){
  let slug='';
  ...
  return (slug||fallback).slice(0,max);
}
...
export function wordPublicSlug(item={}){const fixed=clean(item.publicSlug,150);return fixed?slugifyPublic(fixed,'word',150):`${slugifyPublic(item.gameName,'game')}-${slugifyPublic(item.term||item.title,'word')}`.slice(0,150)}
export function notePublicPath(item={}){return `/notes/${encodeURIComponent(notePublicSlug(item))}/`}
export function wordPublicPath(item={}){const id=publicWordId(item.id||item.sourceGlossaryId||'');return id?`/words/${encodeURIComponent(wordPublicSlug(item))}/`:'/words/'}
...
function noteFromRow(row){
  ...
  const item={id:publicNoteId(row.id),title:row.title||'',body:row.body_text||'',excerpt:row.excerpt||'',gameName:clean(m.gameName,220),typeName:clean(m.typeName,160),sourceNoteId:clean(m.sourceNoteId,180),sourceKind:clean(m.sourceKind,32),sourceClipId:clean(m.sourceClipId,80),tags:list(m.tags,24,80),relatedWaysIds:list(m.relatedWaysIds),sourceCreatedAt:m.sourceCreatedAt||null,publishedAt:m.publishedAt||row.created_at||null,snapshotUpdatedAt:m.snapshotUpdatedAt||row.updated_at||null,publicSlug:clean(m.publicSlug,150),slugHistory:list(m.slugHistory,40,150),seoTitle:clean(m.seoTitle,90)};
...
export async function listPublicNotes(sql){const rows=await sql`SELECT id,title,excerpt,body_text,metadata,created_at,updated_at FROM core.contents WHERE source=${NOTE_PUBLIC_SOURCE} AND content_type=${NOTE_PUBLIC_TYPE} AND status='active' ORDER BY COALESCE((metadata->>'publishedAt')::timestamptz,created_at) DESC`;const notes=rows.map(noteFromRow);const seen=new Map();for(const note of notes){const aliases=[notePublicSlug(note),...(note.slugHistory||[])];for(const slug of aliases){if(seen.has(slug)&&seen.get(slug)!==note.id){const e=new Error('duplicate_public_slug');e.status=409;throw e}seen.set(slug,note.id)}}return notes}
export async function listPublicWords(sql){const rows=await sql`SELECT id,title,excerpt,body_text,metadata,created_at,updated_at FROM core.contents WHERE source=${WORD_PUBLIC_SOURCE} AND content_type=${WORD_PUBLIC_TYPE} AND status='active' ORDER BY COALESCE((metadata->>'publishedAt')::timestamptz,created_at) DESC`;const words=rows.map(wordFromRow),seen=new Map();for(const word of words){for(const alias of [wordPublicSlug(word),...(word.slugHistory||[])]){const slug=slugifyPublic(alias,'word',150);if(seen.has(slug)&&seen.get(slug)!==word.id){const e=new Error('duplicate_public_word_slug');e.status=409;throw e}seen.set(slug,word.id)}}return words}
